  ...
  List<MapEntry<DateTime, double>> _calculateChartData(
    ...
    while (current.isBefore(endDate.add(const Duration(days: 1)))) {
      ...
      if (selectedView == StatisticsView.daily) {
        ...
        // Tạm thời chỉ xử lý daily cho đơn giản, các view khác tương tự
        ...
        BlocListener<Categoriescubit, CategoriesState>(
          listener: (context, state) {
            if (state.status == CategoriesStatus.loaded) {
              setState(() {
                categories = ['Tất cả danh mục', ...state.categoriesExpense.map((c) => c.name)];
              });
  ...
  Widget _buildSummaryHeader(ThemeData theme) {
    return Column(
      ...
      children: [
        Row(
          children: [
            const Text('Chi tiêu', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
      ...
      child: BarChart(
        BarChartData(
          barGroups: chartData.asMap().entries.map((e) {
            return BarChartGroupData(x: e.key, barRods: [BarChartRodData(toY: e.value.value, color: theme.primaryColor, width: 15)]);
          }).toList(),
          ...
          child: PieChart(
            PieChartData(
              sections: pieChartData.map((e) {
                return PieChartSectionData(value: e.value, title: '${e.value.toStringAsFixed(0)}%', color: Colors.blue, radius: 50);
              }).toList(),
            ),
          ),
        ),
